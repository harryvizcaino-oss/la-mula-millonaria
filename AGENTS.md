...
- **Session compose (`src/lib/sessionMult.ts`):** `composeSessionMultiplier({ frenzy, combo, nitro, convoy, event, critical })` — nitro y convoy NO se apilan (`Math.max`); crítico aporta máx ×2 al producto (no el ×10 de display); tope `SESSION_MULT_CAP = 500`. Cableado en `Game.tsx`; autoclick usa path tonto (sin frenzy/combo/tickets/season XP).
...
| `src/pages/Marketplace.tsx` | Store + cash redemption (Gift Card) + Gift Cards RedPostventa (CPS) |
...
- **Layout order (top to bottom):**
  ...
  2. Card "Recargar" (`IapRecargarPanel`) — packs 🎟️ / starter / ad-free / pase COP, **arriba** de Redime.
  3. Card única "Redime" con tabs pill (activo rojo sólido, inactivo gris): tab "Efectivo" (redención de efectivo) y tab "Gift Cards CPS" (grid RedPostventa).
...
- **Gift Cards RedPostventa (CPS):** $10 (100K CPS), $25 (250K), $50 (500K), $100 (1M), $250 (2.5M). Códigos `RPV-...`. Redeeming calls `store.redeemCps(cost)` — only `cpsBalance` decreases; `cpsTotal` (ranking) is untouched.
- **Cash redemption:**
...
- Home (V2-1): hero banner 55vh (`home_banner_juego_completo.jpg`, `.home-banner`, rounded-b 2rem) con overlay gradiente oscuro abajo y partículas doradas; título 3D gold metallic (`.home-title-3d`) sobre el banner; botón JUGAR gold glossy (`.home-play-btn`) solapado bajo el banner (`margin: -28px auto 0`); links gold "Como Jugar?"/"Ver Tienda" (`.home-link-gold`); menú de cuenta en cards limpias (`.home-section-card`); stats panel glassmorphism (`.home-stats-panel`). La sección "Catálogo de premios" del Home muestra 3 productos del feed RedPostventa (`fetchCatalogProducts`) en cards horizontales idénticas a la Tienda (foto 64px + nombre/marca + COP tachado + millas rojas + "Te faltan" + botón flecha roja), con fallback a 4 mocks. Sin fondo blanco/kinder.
...
- **Misiones** (`questStore.ts`, `truckSurfers_quests_v1`): 3 diarias + 1 semanal, tipos `clicks`/`comboTier`/`buyPower`/`collectTickets`/`earnCps`/`buyFleet` (+ `earnTickets`/`spendTickets` como anti-dead-end si flota ≥10). Generación determinística por seed de fecha local (diaria rota cada día, semanal cada lunes). `ensureQuests(ctx?)` rota si cambió el día/semana; `progress(type, amount)` (comboTier guarda el MÁX tier; `collectTickets` también avanza `earnTickets`); `claim(id, ctx?)` escala CPS con `max(base, floor(clickPower * k))` (k daily 20 / weekly 100) y la página aplica la recompensa. UI: `QuestPanel.tsx` (bottom sheet), icono `ClipboardList` en el header flotante de `Game.tsx`.
- **Talentos** (`talentStore.ts`, `truckSurfers_talents_v1`): 12 talentos, 4 ramas × 3 niveles (Poder +5% click/nivel, Combo +0.5s ventana/nivel, Crítico +2%/nivel, Tickets +10% spawn/nivel). Post-max: **Overdrive** infinito (`buyOverdrive`, costo `ceil(5 * 1.5^n)` ⭐, +1% click/nivel vía `getTalentPowerBonus`). Se compran con `stars` vía `clickerStore.spendStars()`; cada nivel requiere el anterior de su rama. Helpers: `getTalentPowerBonus/getTalentComboWindowMs/getTalentCritBonus/getTalentTicketBonus`. `calculateClickPower` y `getCriticalChance` (tope subido a 0.35) los aplican; la ventana de combo es `getComboWindowMs()` en `comboStore.ts`. UI: `TalentTree.tsx` en la pestaña "Talentos" de `Game.tsx`.
- **Logros** (`achievementStore.ts`, `truckSurfers_achievements_v1`): 24 logros (10 poderes max + flota 3/6/10 + CPS 1K/1M/1B/1T + clics 1K/100K/1M + combo x10 + ascensiones 1/10/50). `checkAchievements(snapshot)` marca desbloqueados y devuelve los nuevos; `claim(id, ctx?)` escala CPS levemente con clickPower (`ACHIEVEMENT_CPS_K = 5`). `Game.tsx` checkea en un `useEffect` y muestra `AchievementToast.tsx`; la sección "Logros" de `Profile.tsx` (grid 2 col) permite reclamar.
...
- **F5 Anuncios con recompensa (opt-in):** `src/lib/rewardedAd.ts` (`showRewardedAd()` simulada: 5s en navegador, `false` si se aborta; `true` inmediata fuera del navegador) + `src/components/game/AdRewardModal.tsx` (cuenta regresiva 5s, barra de progreso, botón Cerrar deshabilitado hasta terminar; si se cierra antes NO se entrega nada). Integraciones: `DailyStreak` (botón "Doble recompensa" tras reclamar, prop `onDoubleClaim`), `Game.tsx` (combo roto con 6+ clicks → banner "Revivir combo con anuncio" durante 6s; restaura vía `comboStore.restoreCombo(count)`). Banners/interstitials se apagan con IAP ad-free (`adsConfig.isAdsEnabled()`, `iapStore.isAdFree()`).
- **F6 Pase de temporada "Ruta Nacional":** datos en `src/data/seasonPass.ts` (temporada = trimestre calendario, 30 niveles × 100 XP, track gratis/premium de CPS y 🎟️; CPS base fijos, escalados en claim). Store `src/store/seasonStore.ts` (`truckSurfers_season_v1`): `addXp`, `claimLevel(level, ctx?)` (aplica recompensas al clicker store; CPS `max(base, floor(clickPower * SEASON_CPS_K))` con K=8), `unlockPremium`, `resetSeason`, `hydrateFromServer` (merge: mayor XP gana, claims/premium unión). Premium cuesta 250 🎟️ (se cobra en la página). UI `src/pages/SeasonPass.tsx` (ruta `/season`, acceso desde Profile → "Ruta Nacional"). XP: 1 por click, +25 racha diaria, +50 redenciones (Marketplace efectivo/Gift Cards CPS y Redemption). Sync best-effort: `src/lib/seasonSync.ts` + `src/hooks/useSeasonSync.ts` (debounce 5s, tabla `season_progress`, migración `supabase/migrations/006_season_progress.sql`).
- **F7 Mapa de rutas:** `src/data/routes.ts` (10 ciudades colombianas, requisito de `cpsTotal` de 0 a 10T, bonus +2-5% c/u acumulativo y permanente). Store `src/store/routeStore.ts` (`truckSurfers_routes_v1`): `checkUnlocks(cpsTotal)` (auto-avanza el camión a la ciudad nueva), `setCurrentCity`. UI: pestaña "Ruta" → `NationalMapPanel` (vía `RouteMap.tsx`). El bonus se aplica en `calculateClickPower` vía `computeRouteBonus`.
- **Retos de vía:** `src/data/roadChallenges.ts` (5 mecánicas: climb/descent/toll/storm/port) + 9 tramos entre ciudades. Store `roadChallengeStore` (`truckSurfers_road_challenges_v1`): máx 3 gratis/día, 1 gratis por tramo/día; replay 1 🎟️ en Minijuegos. Primera victoria de tramo → buff +1% click 10 min. UI: `RoadChallengeModal.tsx`. No es el loop principal del clicker.
...
## Wave 4 — Retención y Cosméticos (F13–F16)

- **Recompensas por tiempo de sesión (F13)** (`src/data/sessionRewards.ts` + `src/store/sessionRewardStore.ts`, persist `truckSurfers_session_rewards_v1`): 7 milestones de tiempo activo (1/3/5/10/15/30/60 min) con recompensas crecientes (millas, CPS, 🎟️, power-up aleatorio), reclamables una vez por sesión. La sesión se reinicia tras 10 min de inactividad (`tick` detecta el hueco en `lastActiveAt`). El tiempo lo acumula `useSessionTracker` (intervalo 1s global en `App.tsx`, solo con pestaña visible). UI: bottom sheet `SessionRewardsPanel.tsx` desde el ícono reloj (`Clock`, cyan) de los float-pills de `/game`; la página aplica las recompensas (millas vía MillasProvider).
- **Álbum de coleccionables (F14)** (`src/data/collectibles.ts` + `src/store/collectibleStore.ts`, persist `truckSurfers_collectibles_v1`): 23 coleccionables en 5 sets (Camiones legendarios, Marcas patrocinadoras, Ciudades, Power-ups, Eventos) con rarezas common/rare/epic/legendary (pesos 60/26/11/3). `rollDrop(chance)` / `grantRandom()` / `grant(id)` devuelven `{ def, isNew, setCompleted }`. Fuentes: click 0.4%, comprar poder 5%, tier up 50%, misión reclamada 25%, evento global 50%/15%, minijuego 35%, ascensión garantizada, sobres en el álbum (100K CPS o 25 🎟️, duplicado = +500 M). Bonus por set completo (+1-3% CPS) aplicado en `calculateClickPower` vía `computeAlbumBonus`; completar set también da +5 ⭐ del pase cosmético (lo aplica el propio store). UI: página `/album` (`src/pages/Album.tsx`), acceso desde Profile → "Álbum de Coleccionables".
- **Desafíos semanales globales (F15)** (`src/data/globalChallenges.ts` + `src/store/globalChallengeStore.ts`, persist `truckSurfers_global_challenges_v1`): 3 desafíos de comunidad por semana (lunes 00:00 local, `getWeekKey` de leagues), generados determinísticos por seed semanal (clicks ~1B, CPS ~100T, tickets ~1M). Progreso simulado: pasivo horneado cada 30s (`syncPassive`, ritmo para completar en ~4 días) + aporte del jugador ×1000 (`contribute` desde `clickerStore.click()` y `addGoldenTickets`); `liveCommunity()` calcula el progreso en vivo sin escribir estado. Milestones 25/50/75/100% con recompensas escalonadas (millas, CPS, 🎟️, power-up, coleccionable al 100%) que la UI aplica. UI: página `/challenges` (`src/pages/Challenges.tsx`) con countdown, barras en vivo y contribución personal.
- **Pase premium cosmético (F16)** (`src/data/cosmeticPass.ts` + `src/store/cosmeticPassStore.ts`, persist `truckSurfers_cosmetic_pass_v1`): "Mula Glamour", 20 niveles × 25 estrellas, temporadas de 30 días (`CP-n`). Estrellas: 1/min de sesión (vía `useSessionTracker`), +10 por ascensión, +5 por set del álbum. Track gratis (millas/tickets/coleccionables); premium 500 🎟️ (una vez por temporada, `spendGoldenTickets`) con 10 cosméticos exclusivos SIN bonus de CPS. El store es puro (patrón `leagueStore.claimReward`): `claimLevel` devuelve recompensas y la página las aplica (cosméticos vía `customizationStore.buy`). UI: página `/cosmetic-pass` (`src/pages/CosmeticPass.tsx`), acceso desde Profile → "Mula Glamour".
- **Cosméticos del pase en El Taller (F16):** nuevas categorías en `truckSkins.ts`: `trailEffect` (estela de color bajo el camión, `trailColor` → render en `Game.tsx` sobre el `truck-shadow`) y `avatarFrame` (`frameColor` → borde + glow del avatar en `Profile.tsx`). Piezas con `passExclusive: true` y costo 0: `TruckCustomization` solo las muestra si ya están en `owned` (se ganan en el pase, no se venden). `getTruckVisual` expone `trailColor`/`frameColor`.
- Sin migración Supabase nueva: las 4 features son 100% locales/simuladas (como F9–F12). Futuro: sync de álbum/desafíos/pase si se quiere multijugador real.
